...
zxerr_t crypto_sign(uint8_t *signature,
    ...
    BEGIN_TRY
    {
        TRY
        {
            ...
            cx_keccak_init(&local_sha3, 256);
            cx_hash((cx_hash_t *) &local_sha3, CX_LAST, message, messageLen, messageDigest, CX_SHA256_SIZE);
            ...
            if (info == CX_ECCINFO_PARITY_ODD)
                signature[signatureLength] = 28;
            else
                signature[signatureLength] = 27;
            ++signatureLength;
        }
        CATCH_OTHER(e) {
            err = zxerr_ledger_api_error;
...
zxerr_t crypto_fillAddress(uint8_t *buffer, uint16_t buffer_len, uint16_t *addrResponseLen) {
    ...
    BEGIN_TRY
    {
        TRY {
            ...
            cx_sha3_t local_sha3;
            cx_keccak_init(&local_sha3, 256);
            cx_hash((cx_hash_t *) &local_sha3, CX_LAST, cx_publicKey.W + 1, 64, hashAddress, 32);
            hashAddress[32] = 0;
        }
        CATCH_OTHER(e) {
            err = zxerr_ledger_api_error;
    ...
    CHECK_ZXERR(bech32EncodeFromBytes(addr, buffer_len - PK_LEN_SECP256K1, bech32_hrp, hashAddress + 12, CX_RIPEMD160_SIZE, 1, BECH32_ENCODING_BECH32))
